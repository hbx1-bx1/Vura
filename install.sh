...
if [ -f "${SCRIPT_DIR}/requirements.txt" ]; then
    ...
    echo -e "  ${GREEN}✔${RESET} Core dependencies installed"
else
    echo -e "  ${RED}✘ requirements.txt not found in ${SCRIPT_DIR}${RESET}"
    exit 1
fi

# Install Flet (GUI framework)
echo -e "  ${DIM}Installing Flet GUI framework...${RESET}"
$PY -m pip install --quiet flet
...
if [ \$# -eq 0 ]; then
    ...
    exec python3 \"\${VURA_HOME}/gui.py\" \"\$@\"
else
    # With arguments → launch CLI
    exec python3 \"\${VURA_HOME}/main.py\" \"\$@\"
fi
"
